...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        possibleValues = util.Counter()

        for action in legalActions:
            possibleValues[action] = self.getQValue(state,action)

        return possibleValues[possibleValues.argMax()]
    ...
    def computeActionFromQValues(self, state):
        ...
        if possibleValues.totalCount() == 0:
            return random.choice(legalActions)
        else:
            return possibleValues.argMax()
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        #Here we get the features using the getFeatures extractor
        features = self.featExtractor.getFeatures(state, action)
        #Here we get the weights
        weights = self.getWeights()
        #here we just do as it says up there :D
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        #difference = to reward + gamma * Q(S', a') - Q(s, a)
        ...
        features = self.featExtractor.getFeatures(state, action)
        #weight-updating all the weights and features
        for key in features:
    ...
    def final(self, state):
        ...
        if self.episodesSoFar == self.numTraining:
            #printing weights here for debugging
            "*** YOUR CODE HERE ***"
